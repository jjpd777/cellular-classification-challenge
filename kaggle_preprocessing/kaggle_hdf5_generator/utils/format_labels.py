...
def write_labels_to_csv(train_path,test_path):
    ...
    train["path"] =  "train/"+train["sirna"].astype(str)+"?" + train["filename"]
    write_train = train[["path","sirna"]]
    print(write_train["path"].isna().sum())
    print(write_train["sirna"].isna().sum())
    #test["path"] =  "train/"+test["sirna"].astype(str)+"-" + test["filename"]
